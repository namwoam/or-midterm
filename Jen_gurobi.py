...
def heuristic_algorithm(file_path):
    ...
    for j in N_0:
        for k in N_0:
            if(j != 0 and k != 0):
                D[j][k] = distances[orders[j - 1].return_station - 1][orders[k - 1].pickup_station - 1] / 60
    for i in M:
        for j in N_0:
            if(j != 0):
                A[i][j] = distances[cars[i].station - 1][orders[j - 1].pickup_station - 1] / 60
                if(A[i][j] > 0):
                    A_1[i][j] = 1
    ...
    # objective function
    model.setObjective(quicksum(a[i, j] * P[j - 1] * (R[j-1] - S[j-1]) for i in M for j in N) - 2 * quicksum((1 - quicksum(a[i, j] for i in M)) * P[j - 1] * (R[j-1] - S[j-1]) for j in N), GRB.MAXIMIZE)

    # constraints
    constr = model.addConstrs(quicksum(x[i, j, k] for i in M for j in N_0 if j != k) <= 1 for k in N)
    constr = model.addConstrs(quicksum(x[i, j, k] for i in M for k in N_0 if j != k) <= 1 for j in N)
    constr = model.addConstrs(quicksum(x[i, j, k] for j in N_0 if j != k) == quicksum(x[i, k, h] for h in N_0 if h != k) for i in M for k in N)
    constr = model.addConstrs((R[k-1] - R[j-1]) + V * (1 - x[i, j, k]) >= C_0 + D[j][k] + (R[k-1] - S[k-1]) + C_1 for i in M for j in N for k in N if j != k)
    constr = model.addConstrs(R[k-1] + V * (1 - x[i, 0, k]) >= A[i][k] + (R[k-1] - S[k-1]) + C_1 * A_1[i][k] for i in M for k in N if j != k)
    constr = model.addConstrs(quicksum(x[i, 0, j] for j in N) <= 1 for i in M)
    constr = model.addConstr(quicksum(D[j][k] * x[i, j, k] for i in M for j in N for k in N if j != k) + quicksum(A[i][k] * x[i, 0, k] for i in M for k in N) <= B)
    constr = model.addConstrs(quicksum(x[i, j, k] for k in N_0 if j !=  k) == a[i, j] for i in M for j in N)
    constr = model.addConstrs((L_c[i] - L_o[j-1]) * a[i, j] <= 1 for i in M for j in N)
    constr = model.addConstrs((L_c[i] - L_o[j-1]) * a[i, j] >= 0 for i in M for j in N)
    model.optimize()
                ...
                if x[i, j, k].x == 1:
                    if j == 0:
                        if cars[i].station != orders[k-1].pickup_station:
                            rearrangement.append([i+1, int(cars[i].station), int(orders[k-1].pickup_station), start_time.strftime("%Y/%m/%d %H:%M")])
                    elif orders[j-1].return_station != orders[k-1].pickup_station:
                        rearrangement.append([i+1, int(orders[j-1].return_station), int(orders[k-1].pickup_station), (start_time + timedelta(hours = orders[j-1].return_time + 4)).strftime("%Y/%m/%d %H:%M")])
