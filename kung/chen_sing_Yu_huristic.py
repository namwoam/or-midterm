    ...
    def make_decision(renting_order, this_order, left_order, dash_board, car_level_fee, move, distances):
        ...
        for item in distances[this_order.pickup_station-1]:
            ...
            if (item[0] != 0 and item[0] <= move):  # 如果不是在原站，且沒有超過最大移動量
                ...
                dist = item[0]  # 需要花的時間
                From = item[1]  # 站點
                move_same = dash_board[From -
                                       1][this_order.level-1]  # 看看現在同級的還有多少
                # 看看升級的還有多少
                move_up = dash_board[From-1][this_order.level] if (
                    this_order.level + 1) < n_L else 0
        ...
        # 如果是決定要移車
            ...
            for car in car_station[decision[1]-1]:  # 取車，找同站目前有等級一樣的車
                if (car.car_level == decision[2]):
                    ...
                    detail.append(calculate_start_time(
                        orders[i].pickup_time, decision[3]))  # 真正的開始時間
                    car_station[decision[1]-1].remove(car)
                    break
