...
def heuristic_algorithm(file_path):
    ...
    for i, row in dfs[4].astype(int).iterrows():
        distances[row['From'] - 1].append((row['Distance'], row['To'])) #從第i站出發到(距離,j站)
    ...
    def make_decision(renting_order,this_order,left_order,dash_board,car_level_fee,move,distances):#這個決策變數會決定要"同級","升級","轉單",(從哪裡轉,車子的等級)"放單"(-1)

        #放棄這單的成本
        give_up_cost=loss_caculator(car_level_fee[this_order.level],[this_order])
        same_cost=0
        upgrade_cost=0

        count_same= dash_board[this_order.pickup_station-1][this_order.level-1]-1
        count_upgrade=0
        ...
        if (count_same!=-1):
            ...
            for left in left_order: #檢查所有剩下的單，如果overlap就必須要檢查
                ...
                #如果沒有重疊了，那就結束檢查
                    ...
                    same_cost=loss_caculator(car_level_fee[this_order.level],miss_same)
                    break
        else: #如果不行用同樣的車
        ...
        if (check_upgrade_avalibility(dash_board, this_order)):
            count_upgrade=dash_board[this_order.pickup_station-1][this_order.level]-1
            ...
            for left in left_order: #檢查所有剩下的單，如果overlap就必須要檢查
                ...
                #如果沒有重疊了，那就結束檢查，並計算總損失
                    ...
                    upgrade_cost=loss_caculator(car_level_fee[this_order.level+1],miss_upgrade)
                    break
        else: #如果不行用同樣的車
        ...
        #如果是決定移車(能用原本的就用原本的，能升級就升級，最後再來考量移車)
        for item in distances[this_order.pickup_station-1]:
            
            if (item[0]!=0 and item[0]<=move): #如果不是在原站，且沒有超過最大移動量
                
                dist= item[0] #需要花的時間
                From=item[1] #站點
                move_same=dash_board[From-1][this_order.level-1] #看看現在同級的還有多少
                move_up = dash_board[From-1][this_order.level] if (this_order.level + 1) < n_L else 0 #看看升級的還有多少


                #如果目前可以從其他地方移同levle的車
                for previous in renting_order: #如果之前結束的單，但是加上移動時間後兩個時間會overlap
                        if (From == previous.return_station and check_totally_after(previous, this_order) and check_overlap(previous,this_order,4.5+dist)):
                            if (previous.level==this_order.level): #levle 一樣
                                move_same-=1 #扣除
                            elif (previous.level==this_order.level+1):
                                move_up-=1

                if (move_same>=1): #如果可以移就移
                    move_from=From
                    car_level=this_order.level
                    break
                elif (move_up>=1):
                    move_from=From
                    car_level=this_order.level+1
                    break
        if (move_from!=0): #如果可以移的話就移
            return (3,move_from,car_level,dist)
    ...
    for i in range(0,n_K):
        ...
        while(len(take_order)!=0):
            if check_totally_after(take_order[0], orders[i]):
                dash_board[take_order[0].return_station-1][renting_car[0].car_level-1]+=1
                car_station[take_order[0].return_station-1].append(renting_car.pop(0))
       ...
       #如果是選擇同站同車
        ...
        #如果是決定要移車
            ...
            detail=[]
            take_order.append(orders[i]) #決定選這單
            dash_board[decision[1]-1][decision[2]-1]-=1 #這類車在這個站點的數量扣1
            for car in car_station[decision[1]-1]:     #取車，找同站目前有等級一樣的車
                if (car.car_level == decision[2]):
                    renting_car.append(car) #把它加到目前租借
                    assignment[i]=car.car_id #把他加道計畫中
                    move-=decision[3]#移動的code先扣掉。
                    detail.append(car.car_id) #紀錄車子的數量
                    detail.append(decision[1]) #從i開始搬
                    detail.append(orders[i].pickup_station) #搬到j
                    detail.append(calculate_start_time(orders[i].pickup_time,decision[3])) #真正的開始時間
                    car_station[decision[1]-1].remove(car)
                    break
